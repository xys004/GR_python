...
# ==============================================================================
# SECTION 4 — PARALLEL COMPUTATION HELPERS
# (Linux / Google Colab only — NOT compatible with Windows due to spawn vs fork)
# ==============================================================================


def _christoffel_worker(args):
    """
    Top-level worker for ProcessPoolExecutor.
    Computes Gamma^lam_{mu nu} for one (lam, mu, nu) triple.
    Returns a list of ((lam, mu, nu), value) pairs (exploits mu<->nu symmetry).

    All arguments are passed as a single tuple to satisfy the single-argument
    requirement of ProcessPoolExecutor.map.
    """
    lam, mu, nu, g_list, ginv_list, coord_strs, dim = args
    from sympy import symbols, diff, cancel, Matrix
    # Reconstruct SymPy objects from picklable (list) representations
    coords_s = [symbols(c) for c in coord_strs]
    g    = Matrix(dim, dim, g_list)
    ginv = Matrix(dim, dim, ginv_list)
    val = sum(
        ginv[lam, sig] * (
            diff(g[nu, sig], coords_s[mu]) +
            diff(g[mu, sig], coords_s[nu]) -
            diff(g[mu, nu], coords_s[sig])
        ) / 2
        for sig in range(dim)
    )
    val = cancel(val)
    result_pairs = [((lam, mu, nu), val)]
    if mu != nu:
        result_pairs.append(((lam, nu, mu), val))   # exploit lower-index symmetry
    return result_pairs


def compute_christoffel_parallel(g, ginv, coords, dim=4, n_jobs=4):
    """
    Parallel version of compute_christoffel using ProcessPoolExecutor.

    Distributes independent (lam, mu, nu) triples across n_jobs worker processes.
    Exploits the symmetry Gamma^lam_{mu nu} = Gamma^lam_{nu mu} to halve the work.

    Only use on Linux / Google Colab.  On Windows, use compute_christoffel instead
    (Windows uses 'spawn' start method which cannot pickle SymPy expressions).

    Parameters
    ----------
    g, ginv  : sympy.Matrix   — covariant and contravariant metric
    coords   : list           — coordinate symbols
    dim      : int            — spacetime dimension
    ...
    Returns
    -------
    Gamma : 3-D list  Gamma[lam][mu][nu]
    """
    from concurrent.futures import ProcessPoolExecutor
    # Serialise SymPy Matrices to plain Python lists for safe pickling
    g_list     = [g[i, j]    for i in range(dim) for j in range(dim)]
    ginv_list  = [ginv[i, j] for i in range(dim) for j in range(dim)]
    coord_strs = [str(c) for c in coords]
    # Only generate upper-triangle tasks (mu <= nu); worker fills both halves
    tasks = [
        (lam, mu, nu, g_list, ginv_list, coord_strs, dim)
        for lam in range(dim)
        for mu  in range(dim)
        for nu  in range(mu, dim)
    ]
    ...
    with ProcessPoolExecutor(max_workers=n_jobs) as ex:
        for pairs in ex.map(_christoffel_worker, tasks):
            for (lam, mu, nu), val in pairs:
                Gamma[lam][mu][nu] = val
                Gamma[lam][nu][mu] = val   # symmetry
    return Gamma


def _riemann_worker(args):
    """
    Top-level worker for ProcessPoolExecutor.
    Computes R^rho_{sig mu nu} for one (rho, sig, mu, nu) quadruple.
    """
    rho, sig, mu, nu, Gamma_flat, dim, coord_strs = args
    from sympy import symbols, diff, cancel
    coords_s = [symbols(c) for c in coord_strs]

    # Reconstruct Gamma from flat list:  Gamma[a][b][c] = Gamma_flat[a*dim*dim + b*dim + c]
    def G(a, b, c):
        return Gamma_flat[a * dim * dim + b * dim + c]

    val = (
        diff(G(rho, nu, sig), coords_s[mu]) -
        diff(G(rho, mu, sig), coords_s[nu]) +
        sum(
    ...
    val = cancel(val)
    return (rho, sig, mu, nu), val


def compute_riemann_parallel(Gamma, coords, dim=4, n_jobs=4):
    """
    Parallel version of compute_riemann using ProcessPoolExecutor.

    Distributes all dim^4 (rho, sig, mu, nu) quadruples across n_jobs workers.

    Only use on Linux / Google Colab.  On Windows, use compute_riemann instead.
    ...
    Returns
    -------
    R : 4-D list  R[rho][sig][mu][nu]
    """
    from concurrent.futures import ProcessPoolExecutor
    # Flatten Gamma to a plain list for pickling
    Gamma_flat = [
        Gamma[a][b][c]
        for a in range(dim)
        for b in range(dim)
    ...
    coord_strs = [str(c) for c in coords]
    tasks = [
        (rho, sig, mu, nu, Gamma_flat, dim, coord_strs)
        for rho in range(dim)
        for sig in range(dim)
        for mu  in range(dim)
        for nu  in range(dim)
    ]
    R = [[[[None] * dim for _ in range(dim)] for _ in range(dim)] for _ in range(dim)]
    with ProcessPoolExecutor(max_workers=n_jobs) as ex:
        for (rho, sig, mu, nu), val in ex.map(_riemann_worker, tasks):
            R[rho][sig][mu][nu] = val
    return R
# ==============================================================================